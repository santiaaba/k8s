				...
				while (i < volumes.length && !encontre){
					if(volumes[i].name == v.name){
						...
						if(typeof(volumes[i].persistentVolumeClaim) != 'undefined'){
							/* Es un PVC */
							data.volumes.push({
							name:v.name,path:v.mountPath,type:'pvc'})
